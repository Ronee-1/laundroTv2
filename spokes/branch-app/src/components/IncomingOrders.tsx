...
export function IncomingOrders({ selectedAdminBranch, triggerNotification }: Props) {
  ...
  async function handleAssignToCourier(order: IncomingOrder) {
    ...
    try {
      ...
      const res = await fetch(`/api/orders/${order.id_order}/status`, {
        method: 'PATCH',
      ...
      if (!res.ok || !json.success) {
        triggerNotification(json.message ?? 'Gagal memproses pesanan.', 'error');
        return;
      }
      ...
      triggerNotification(
        `✓ Pesanan ${order.id_order}交给了 ${courierName}. Kurir akan melihat tugas di aplikasi mobile.`,
        'success'
      );

      // Remove from incoming list and refresh
      setOrders(prev => prev.filter(o => o.id_order !== order.id_order));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <button
                key={order.id_order}
                onClick={() => setSelectedOrder(selectedOrder?.id_order === order.id_order ? null : order)}
                className={`w-full bg-white border rounded-2xl p-5 text-left transition-all hover:shadow-md ${
          ...
          {/* Detail Panel */}
          {selectedOrder && (
            <div className="bg-white border border-deep-blue rounded-2xl p-6 h-fit sticky top-4">
