...
export async function POST(
  ...
  try {
    ...
    console.log(`[BRANCH ORDER] Created: ${order.id_order}`);
    console.log(`[BRANCH ORDER] metode_pembayaran: ${order.metode_pembayaran}`);
    console.log(`[BRANCH ORDER] total_harga: ${order.total_harga}`);
    console.log(`[BRANCH ORDER] created_at: ${order.created_at}`);
    ...
    if (order.metode_pembayaran === 'Tunai' && order.total_harga > 0) {
      ...
      console.log(`[BRANCH ORDER] Cashbook entry created for Tunai payment`);
    } else {
      console.log(`[BRANCH ORDER] No cashbook entry (Non-Tunai or zero amount)`);
    }
