...
router.post(
  '/request',
  async (req: Request<{}, ExpenseResponse, ExpenseRequestBody>, res: Response<ExpenseResponse>) => {
    try {
      ...
      const budgetCheck = await checkOverbudget(id_cabang, nominal);
...
router.patch(
  '/:id_expense/approve',
  async (req: Request<ApproveParams, ApproveResponse, ApproveBody>, res: Response<ApproveResponse>) => {
    try {
      ...
      const expense = await getExpenseById(id_expense);
      if (!expense) {
        res.status(404).json({
          success: false,
      ...
      const branch = await getBranchById(expense.id_cabang);

      if (status === 'Approve') {
        const deducted = await deductBudget(expense.id_cabang, expense.nominal);
        ...
        const updated = await updateExpenseStatus(id_expense, 'Approve', catatan);

        const budgetCheck = await checkOverbudget(expense.id_cabang, 0);
      ...
      const updated = await updateExpenseStatus(id_expense, 'Reject', catatan);
