...
router.post('/login', async (req, res) => {
    try {
        ...
        if (user.role === 'Kurir' && user.id_cabang) {
            const courier = await prisma_js_1.prisma.courier.findFirst({
                where: { id_cabang: user.id_cabang },
 ...
 */
router.post('/register', auth_js_1.authenticateToken, auth_js_1.requireOwner, async (req, res) => {
    try {
        const { nama, email, password, role, id_cabang } = req.body;
        // ============================================================
        // VALIDATION
        // ============================================================
        const errors = [];
        ...
        if (role === 'Admin') {
            const existingAdmin = await prisma_js_1.prisma.user.findFirst({
                where: {
                    id_cabang: id_cabang,
                    role: 'Admin',
                    is_active: true,
 ...
 */
router.get('/users', auth_js_1.authenticateToken, auth_js_1.requireOwner, async (_req, res) => {
    try {
 ...
 */
router.delete('/users/:id', auth_js_1.authenticateToken, auth_js_1.requireOwner, async (req, res) => {
    try {
        const { id } = req.params;
        const user = await prisma_js_1.prisma.user.findUnique({
            where: { id_user: id },
 ...
 */
router.get('/branches', auth_js_1.authenticateToken, auth_js_1.requireOwner, async (_req, res) => {
    try {
