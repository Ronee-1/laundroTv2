...
router.post('/request', async (req, res) => {
    try {
        ...
        const budgetCheck = await (0, budget_js_1.checkOverbudget)(id_cabang, nominal);
        ...
        const expense = await (0, expense_js_1.createExpense)({
            id_cabang,
            tanggal: parsedDate,
            nominal,
            deskripsi,
        ...
        console.error('[Expenses] POST /request error:', error);
        res.status(500).json({
            success: false,
...
router.patch('/:id_expense/approve', async (req, res) => {
    try {
        ...
        const expense = await (0, expense_js_1.getExpenseById)(id_expense);
        if (!expense) {
            res.status(404).json({
                success: false,
        ...
        const branch = await (0, branches_js_1.getBranchById)(expense.id_cabang);
        if (status === 'Approve') {
            const deducted = await (0, budget_js_1.deductBudget)(expense.id_cabang, expense.nominal);
            if (!deducted) {
                res.status(400).json({
                    success: false,
            ...
            const journal = await (0, cashbook_js_1.createJournalEntry)({
                id_cabang: expense.id_cabang,
            ...
            const updated = await (0, expense_js_1.updateExpenseStatus)(id_expense, 'Approve', catatan);
            const budgetCheck = await (0, budget_js_1.checkOverbudget)(expense.id_cabang, 0);
            res.status(200).json({
                success: true,
        ...
        const updated = await (0, expense_js_1.updateExpenseStatus)(id_expense, 'Reject', catatan);
        res.status(200).json({
            success: true,
            id_expense: updated.id_expense,
            id_cabang: updated.id_cabang,
        ...
        console.error('[Expenses] PATCH /approve error:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error',
        });
